...
export function createToolbarControls(
	...
	return waitForToolbar().then( ( $sections ) => {
		...
		if ( $existing.length ) {
			const $existingCheckbox = $existing.find<HTMLInputElement>( 'input[type="checkbox"]' );
...
export function setControlsVisible( controls: ToolbarControls | null, visible: boolean ): void {
	if ( !controls ) {
		return;
	}
...
export function isCodeMirrorVisible(): boolean {
	...
	return $wrapper.length > 0 && !$wrapper.hasClass( 'ext-codemirror-wrapper--hidden' );
}

export function shouldShowControls(): boolean {
	return !isSectionEdit() && isCodeMirrorVisible();
}
