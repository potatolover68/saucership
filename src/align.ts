...
export function tokensToRanges(
  ...
  return mergeRanges(ranges, tokens.map((token) => token.str).join(""));
}

export function collectInsertRanges(
  before: string,
  after: string,
): TextRange[] {
  if (before === after) {
...
export function rangesToTokens(
  text: string,
  userRanges: TextRange[],
  userClass: string,
): WhoColorToken[] {
  const owned = new Uint8Array(text.length);
  for (let r = 0; r < userRanges.length; r++) {
    const from = userRanges[r].from < 0 ? 0 : userRanges[r].from;
    const to = userRanges[r].to > text.length ? text.length : userRanges[r].to;
    for (let i = from; i < to; i++) {
      owned[i] = 1;
    }
  }
  const tokens: WhoColorToken[] = [];
  let i = 0;
  while (i < text.length) {
    const flag = owned[i];
    let j = i + 1;
    while (j < text.length && owned[j] === flag) {
      j++;
    }
    tokens.push({
      str: text.slice(i, j),
      className: flag ? userClass : "",
    });
    i = j;
  }
  return tokens;
