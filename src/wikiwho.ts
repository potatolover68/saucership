...
function parseEditor( entry: unknown ): { editorName: string; className: string } | null {
...
function parseResponse( json: unknown ): WhoColorData {
	...
	const rawEditors = Array.isArray( rec.present_editors ) ? rec.present_editors : [];
	...
	if ( presentEditors.length !== rawEditors.length ) {
		debugLog( `dropped ${ rawEditors.length - presentEditors.length } present_editors during parse` );
	...
	return {
		...
		pageTitle: typeof rec.page_title === 'string' ? rec.page_title : pageTitle(),
...
export function findEditorClassName( data: WhoColorData, userName: string ): string | null {
...
export async function getWhoColor(): Promise<WhoColorData> {
	...
	const request = ( async () => {
		const url = 'https://wikiwho.wmcloud.org/en/whocolor/v1.0.0-beta/' +
