...
let highlightMark: { range: ( from: number, to: number ) => unknown } | null = null;
...
async function loadLib(): Promise<CodeMirrorLib> {
	...
	const requireFn = await mw.loader.using( 'ext.CodeMirror.lib' ) as ( module: string ) => CodeMirrorLib;
	lib = typeof requireFn === 'function' ?
		requireFn( 'ext.CodeMirror.lib' ) :
		requireFn as unknown as CodeMirrorLib;
	debugLog(
		'CodeMirror.lib loaded',
		lib ? Object.keys( lib as unknown as Record<string, unknown> ).slice( 0, 40 ) : lib
	);
	...
	if ( lib.ViewPlugin && lib.ViewPlugin.define ) {
		parts.push( lib.ViewPlugin.define(
			function ( view ) {
				...
				const value: PluginValue = {
					...
					update: function ( update ) {
						...
						if ( seenGeneration !== rangesGeneration ) {
							...
							value.decorations = ( value.decorations as DecorationSetLike )
								.map( update.changes );
						}
					}
				};
	...
	if ( lib.EditorView && lib.EditorView.theme ) {
		parts.push( lib.EditorView.theme( {
			'.saucership-author': {
...
export async function ensureHighlightExtension( cm: CodeMirrorInstance ): Promise<void> {
	await loadLib();
...
export async function setHighlightRanges(
	...
	const spanTo = currentRanges.length ? currentRanges[ currentRanges.length - 1 ].to : 0;
	debugLog(
...
export async function clearHighlightRanges( cm: CodeMirrorInstance | null ): Promise<void> {
	currentRanges = [];
	rangesGeneration++;
